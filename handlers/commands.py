...
def chat_member(update: Update, context: CallbackContext) -> None:
    ...
    for member in new_members:
        user_id = member.id
        #join_date = datetime.utcfromtimestamp(member.date).strftime('%Y-%m-%d %H:%M:%S')
        join_date = datetime.now(moscow_tz)
        user_join_dates[user_id] = join_date
        # Print or use the join date as needed
        logger.info(f"User {user_id} joined the chat on {join_date.strftime('%Y-%m-%d %H:%M:%S')}")
...
def text_message(update, context):
    ...
    if user_id in user_join_dates:
        ...
        logger.info(f"User {user_id} joined the chat ({chat_id}) on {join_date.strftime('%Y-%m-%d %H:%M:%S')}, time_difference={time_difference}, is_less_one_day={is_less_one_day}, message_has_links={message_has_links}")
    ...
    # For example, you can reply to the user with the same message
